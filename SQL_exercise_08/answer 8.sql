...
Select *From Department;
 ...
 -- 8.1 Obtain the names of all physicians that have performed a medical procedure 
 -- they have never been certified to perform.
 
 Select Physician.Name  From Physician Inner Join Undergoes On Physician.name = Undergoes.Procedure;
 
 Select Name From Physician Where EmployeeID In(Select physician from undergoes where not exists
 (select *From Trained_In Where Treatment = Procedures and physician = Undergoes.Physician));
  
  
SELECT P.NAME AS Physician, Pr.NAME AS `procedure`, U.DateUndergoes,Pt.NAME AS Patient
FROM Physician P, Undergoes U, Patient Pt, Procedures Pr WHERE U.Patient = Pt.SSN AND U.Procedures = Pr.CODE
AND U.Physician = P.EmployeeID AND NOT EXISTS ( SELECT * FROM Trained_In T WHERE T.Treatment = U.Procedures 
AND T.Physician = U.Physician );

-- 8.2 Same as the previous query, but include the following information in the results: 
-- Physician name, name of procedure, date when the procedure was carried out, name of the 
-- patient the procedure was carried out on.

 Select Physician.Name, Procedure.Code, Udergoes.Date Patient.SSN From physician Join  

 
-- 8.3 Obtain the names of all physicians that have performed a medical procedure that they 
-- are certified to perform, but such that the procedure was done at a date (Undergoes.Date) after 
-- the physician's certification expired (Trained_In.CertificationExpires).


-- 8.4 Same as the previous query, but include the following information in the results: Physician name, 
-- name of procedure, date when the procedure was carried out, name of the patient the procedure was carried 
-- out on, and date when the certification expired.

-- 8.5 Obtain the information for appointments where a patient met with a 
-- physician other than his/her primary care physician. Show the following information: 
...
-- specifically, the Undergoes table may include a row where the patient ID does not match the one 
...
-- 8.8 The hospital has several examination rooms where appointments take place. Obtain 
-- the number of appointments that have taken place in each examination room.

-- 8.9 Obtain the names of all patients (also include, for each patient, the name of the patient's 
